...
function dcRenderWzrost(){
  ...
  const h=G.cHist||[];
  if(!h.length){
    el.innerHTML='<div style="font-size:var(--fs-body);color:var(--gr);padding:20px 10px;text-align:center;line-height:1.6;">'+t('dc_wzrost_empty')+'</div>';
    return;
  }
  el.innerHTML='';
  ...
  sec1.textContent=t('dc_sec_reputation');el.appendChild(sec1);
  ...
  sec2.textContent=t('dc_sec_stadium_cap');el.appendChild(sec2);
  ...
  sec3.textContent=t('dc_sec_budget');el.appendChild(sec3);
...
function dcRenderLegенды(){
  ...
  if(candidates.length){
    ...
    prog.innerHTML='<div class="leg-prog-title">'+t('dc_leg_candidates_title')+'</div>';
  ...
  if(!legends.length&&!candidates.length){
    ...
    nd.textContent=t('dc_leg_none');
    el.appendChild(nd);
  }
  if(!legends.length&&candidates.length){
    const nd=document.createElement('div');
    nd.style.cssText='font-size:var(--fs-body);color:var(--gr);text-align:center;padding:8px 0 14px;';
    nd.textContent=t('dc_leg_none_threshold').replace('{n}',LEG_THRESHOLD);
    el.appendChild(nd);
  }
  ...
  function buildLegCard(s,i,rankEmoji,rankColor,cardClass,isAcad){
    ...
    if(s.isRetired)badges.push('<span class="leg-badge" style="border-color:var(--gr);color:var(--gr)">'+t('dc_leg_badge_retired')+'</span>');
    if(isAcad)badges.push('<span class="leg-badge" style="border-color:#9c27b0;color:#ce93d8">'+t('dc_leg_badge_academy')+'</span>');
    ...
    const extraStat=isAcad
      ?'<div><div class="leg-sl">'+t('dc_leg_stat_debut')+'</div><div class="leg-sv">'+(debutSeason?'S'+debutSeason:'?')+'</div></div>'
      :'<div><div class="leg-sl">'+t('dc_leg_stat_titles')+'</div><div class="leg-sv" style="color:var(--am)">'+(s.leagues||0)+'</div></div>';

    card.innerHTML=
      '<div class="leg-rank">'+rankEmoji+' #'+(i+1)+'</div>'+
      '<div class="leg-name" style="padding-right:40px">'+nameLink+'</div>'+
      '<div class="leg-stats">'+
        '<div><div class="leg-sl">'+t('dc_leg_stat_matches')+'</div><div class="leg-sv">'+(s.matches||0)+'</div></div>'+
        '<div><div class="leg-sl">'+t('dc_leg_stat_goals')+'</div><div class="leg-sv">'+(s.goals||0)+'</div></div>'+
        '<div><div class="leg-sl">'+t('dc_leg_stat_assists')+'</div><div class="leg-sv">'+(s.assists||0)+'</div></div>'+
        extraStat+
      '</div>'+
      (badges.length?'<div class="leg-badges">'+badges.join('')+'</div>':'')+
      '<div class="leg-score-row">'+
        '<span class="leg-score-lbl">'+t('dc_leg_score_label')+'</span>'+
  ...
  if(mainLegs.length){
    ...
    sec.textContent=t('dc_leg_top_title');
    el.appendChild(sec);
  ...
  if(acadLegs.length){
    ...
    sec2.textContent=t('dc_leg_academy_top_title');
    el.appendChild(sec2);
  ...
  info.innerHTML=
    '<div class="leg-info-title">'+t('dc_leg_info_title')+'</div>'+
    '<div class="leg-info-row">'+t('dc_leg_info_threshold').replace('{n}',LEG_THRESHOLD).replace(/^([^:]+:) (.*)$/,'$1 <span>$2</span>')+'</div>'+
    '<div class="leg-info-row">'+t('dc_leg_info_points').replace('{w}',LEG_W).replace('{g}',LEG_G).replace('{a}',LEG_A).replace(/\+([\d.]+ pk?ts?)/g,'<span>+$1</span>')+'</div>'+
    '<div class="leg-info-row">'+t('dc_leg_info_trophy_points').replace('{m}',LEG_M).replace('{p}',LEG_P).replace(/\+([\d.]+ pk?ts?)/g,'<span>+$1</span>')+'</div>'+
    '<div class="leg-info-row" style="margin-top:5px;color:#546e54">'+t('dc_leg_info_example').replace('{n}',LEG_THRESHOLD)+'</div>';
  el.appendChild(info);

  const rew=document.createElement('div');rew.className='leg-rewards';
  rew.innerHTML=
    '<div class="leg-reward-title">'+t('dc_leg_rewards_title')+'</div>'+
    '<div class="leg-reward-row"><span>⭐</span> '+t('dc_leg_reward_history')+'</div>'+
    '<div class="leg-reward-row"><span>+15</span> '+t('dc_leg_reward_rep')+'</div>'+
    '<div class="leg-reward-row"><span>⭐</span> '+t('dc_leg_reward_icon')+'</div>';
  el.appendChild(rew);
}
...
function dcRenderKadra(){
  ...
  kpi.innerHTML=
    '<div class="dc-kpi-box"><span class="dc-kpi-v" style="color:#9c27b0">'+fmt(squadVal)+'</span><div class="dc-kpi-l">'+t('dc_kadra_value_now')+'</div></div>'+
    '<div class="dc-kpi-box"><span class="dc-kpi-v" style="color:var(--am)">'+myPl().length+'</span><div class="dc-kpi-l">'+t('dc_kadra_players_count')+'</div></div>';
  el.appendChild(kpi);
  ...
  const sec2=document.createElement('div');sec2.className='dc-sec';sec2.style.color='#ce93d8';sec2.style.borderColor='#9c27b0';sec2.textContent=t('dc_kadra_ovr_pot_title');el.appendChild(sec2);

  const players=myPl().slice().sort((a,b)=>ovr(b)-ovr(a));
  const tbl=document.createElement('table');tbl.className='dc-table';
  tbl.innerHTML='<thead><tr><th>'+t('dc_kadra_col_player')+'</th><th>'+t('dc_kadra_col_pos')+'</th><th>'+t('dc_kadra_col_ovr')+'</th><th>'+t('dc_kadra_col_pot')+'</th><th>'+t('dc_kadra_col_delta')+'</th><th>'+t('dc_kadra_col_value')+'</th></tr></thead>';
  ...
  if(grads.length){
    const sec3=document.createElement('div');sec3.className='dc-sec';sec3.style.color='#ce93d8';sec3.style.borderColor='#9c27b0';sec3.textContent=t('dc_kadra_academy_grads_title');el.appendChild(sec3);
    grads.forEach(p=>{
      ...
      card.innerHTML='<div><div style="font-size:var(--fs-body);color:var(--wh)">'+p.name+' <span style="color:#9c27b0;font-size:var(--fs-meta)">🎓</span></div>'+
...
function dcRenderKlub(){
  ...
  const LBL={'W':t('dc_klub_legend_win'),'R':t('dc_klub_legend_draw'),'P':t('dc_klub_legend_loss')};

  // Legenda
  el.innerHTML=
    '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:10px">'+
    '<span><span style="display:inline-block;width:11px;height:11px;background:var(--gb);border-radius:2px;margin-right:3px;vertical-align:middle"></span><span style="font-size:var(--fs-body);color:var(--gr)">'+t('dc_klub_legend_win')+'</span></span>'+
    '<span><span style="display:inline-block;width:11px;height:11px;background:var(--am);border-radius:2px;margin-right:3px;vertical-align:middle"></span><span style="font-size:var(--fs-body);color:var(--gr)">'+t('dc_klub_legend_draw')+'</span></span>'+
    '<span><span style="display:inline-block;width:11px;height:11px;background:var(--rd);border-radius:2px;margin-right:3px;vertical-align:middle"></span><span style="font-size:var(--fs-body);color:var(--gr)">'+t('dc_klub_legend_loss')+'</span></span>'+
    '<span style="font-size:var(--fs-body);color:var(--gr)">'+t('dc_klub_legend_homeaway')+'</span>'+
    '</div>';
  ...
  function makeDot(res,isHome,rnd,opp,mg,og){
    ...
    dot.innerHTML='<div class="dc-htip">'+t('dc_klub_tooltip').replace('{rnd}',rnd).replace('{venue}',isHome?'🏠':'✈').replace('{opp}',opp).replace('{mg}',mg).replace('{og}',og).replace('{result}',LBL[res])+'</div>';
    return dot;
  }
  ...
  seasons.forEach((d,di)=>{
    ...
    hdr.innerHTML=
      ...
      (pos?'<span style="font-weight:700;font-size:var(--fs-micro);color:'+posColor+'">'+t('dc_klub_place_suffix').replace('{pos}',pos)+'</span>':
           '<span style="font-size:var(--fs-body);color:var(--gr)">'+t('dc_klub_in_progress')+'</span>')+
      (d.pts?'<span style="font-size:var(--fs-body);color:var(--gr)">'+t('dc_klub_pts_suffix').replace('{n}',d.pts)+'</span>':'')+
      (isChamp?'<span style="font-size:var(--fs-body)">🏆</span>':'')+
      (isCup?'<span style="font-size:var(--fs-body)">🥇</span>':'');
    el.appendChild(hdr);
    ...
    if(matches.length){
      ...
      if(!total){
        ...
        nd.textContent=t('dc_klub_no_match_data');el.appendChild(nd);
      } else {
        ...
        results.forEach(res=>{
          ...
          dot.style.background=COL[res];
          dot.innerHTML='<div class="dc-htip">'+t('dc_klub_heat_tooltip').replace('{result}',LBL[res]).replace('{n}',d.season)+'</div>';
      ...
      const nd=document.createElement('div');
      nd.style.cssText='font-size:var(--fs-body);color:var(--gr);padding:2px 0 6px';
      nd.textContent=t('dc_klub_no_matches_played');el.appendChild(nd);
    }
  });
}
